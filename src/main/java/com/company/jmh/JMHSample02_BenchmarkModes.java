...
public class JMHSample02_BenchmarkModes {

	/*
	 * JMH generates lots of synthetic code for the benchmarks for you during the
	 * benchmark compilation. JMH can measure the benchmark methods in lots of
	 * modes. Users may select the default benchmark mode with a special annotation,
	 * or select/override the mode via the runtime options.
	 *
	 * With this scenario, we start to measure something useful. Note that our
	 * payload code potentially throws exceptions, and we can just declare them to
	 * be thrown. If the code throws the actual exception, the benchmark execution
	 * will stop with an error.
	 *
	 * When you are puzzled with some particular behavior, it usually helps to look
	 * into the generated code. You might see the code is doing not something you
	 * intend it to do. Good experiments always follow up on the experimental setup,
     ...
     *    $ mvn clean install
     *    $ java -jar target/benchmarks.jar JMHSample02_BenchmarkModes -f 1
     *    
     *    
     */
}
